...
(level_width, level_height) = (31, 18)  # dimensions of world in blocks
block_pixels = 32  # size of (square) block sprites in pixels
(window_block_width, window_block_height) = (31, 18)
(window_pixel_width, window_pixel_height) = (
    window_block_width * block_pixels,
    window_block_height * block_pixels,
)
window_scale = 1
...
FOOD_IMAGE = load_image("4-food.png")
SPLAT_IMAGE = load_image("a-loser.png")
...
app_icon = load_image("levels/9-matt.png")
...
class WincollGame:
    ...
    def restart_level(self) -> None:
        ...
        w, h = window_block_width * block_pixels, window_block_height * block_pixels
    ...
    def save_position(self) -> None:
        self.set(self.hero.position, Tile.MATT)
    ...
    def survey(self) -> None:
        """Count food on level and find start position."""
        self.food = 0
    ...
    # PROC move:(dx%,dy%,stopX%,stopY%)
    # 	EXTERNAL m&,mx%,my%,world&,worldBot&,apples%,moves%,wrong%,key&,flash%,redisplay%
    # 	EXTERNAL soundDir$,volume%,soundThread&
    # 	LOCAL om&,nm&,nx%,ny%,stopM&,d%,o%,push%
    # 	CURSOR OFF : flash%=KFalse% : REM Stop Mr Matt flashing
    # 	d%=dx%+(dy%*cols%)
    # 	stopM&=world&+stopX%+stopY%*cols%
    # 	DO
    # 		om&=m& : nm&=m&+d% : nx%=mx%+dx% : ny%=my%+dy%
    # 		IF nx%<0 OR nx%=cols% OR ny%<0 OR ny%=rows% : BREAK : ENDIF
    # 		o%=PEEKB(nm&)
    # 		push%=o%>=rock% AND ABS(d%)=1 AND nx%>0 AND nx%<cols%-1 AND PEEKB(nm&+d%)=empty%
    # 		IF o%<wall% OR push%
    # 			POKEB m&,empty% : POKEB nm&,matt%
    # 			plotBit:(mx%,my%,empty%)
    # 			m&=nm& : mx%=nx% : my%=ny%
    # 			IF push%
    # 				POKEB m&+d%,o%
    ...
    # 			IF o%=apple%
    # 				apples%=apples%-1
    # 				sound:(soundDir$+"pick",volume%,ADDR(soundThread&))
    # 			ENDIF
    # 			IF push% AND nm&<worldBot& AND (PEEKB(m&+d%+cols%)=empty% OR PEEKB(m&+d%+cols%)>=box3%)
    # 				fall:(m&+d%,d%)
    # 			ENDIF
    # 			IF d%<>-cols% AND my%
    # 				fall:(m&-d%-cols%,0)
    ...
    # 			moves%=moves%+1
    # 		ENDIF
    # 	UNTIL key&<>KEvPtr& OR m&=stopM& OR m&=om& OR wrong% OR apples%=0
    # 	redisplay%=KFalse%
    # ENDP

    # PROC fall:(p&,pushDir%)
    # 	REM The first rock to fall (if any) must have a space below
    # 	EXTERNAL world&
    # 	LOCAL q&,dir%
    # 	q&=p& : dir%=pushDir%
    # 	WHILE PEEKB(q&)>=rock% AND q&>=world&
    # 		plummet:(q&,dir%)
    # 		IF q&<cols% : BREAK : ENDIF
    # 		q&=q&-cols%
    # 		dir%=0
    # 	ENDWH
    # ENDP

    # PROC plummet:(p&,pushDir%)
    # 	EXTERNAL world&,worldBot&,wrong%,soundDir$,volume%,soundThread&,mx%
    # 	LOCAL q&,nq&,x%,o%,this%,dir%
    # 	q&=p& : x%=SyMod&:(q&-world&,cols%)
    # 	dir%=pushDir%
    # 	this%=PEEKB(p&)
    # 	POKEB p&,empty%
    # 	DO
    ...
    # 		IF o%=empty%
    # 			nq&=q&+cols%
    # 			IF nq&<worldBot& : IF PEEKB(nq&+cols%)=matt% : wrong%=KTrue% : ENDIF : ENDIF
    # 		ELSEIF o%>=box3%
    # 			q&=q&+cols% : this%=o%+1
    # 			IF this%>box1%
    # 				this%=empty%
    # 				sound:(soundDir$+"debox",volume%,ADDR(soundThread&))
    # 			ELSE
    # 				sound:(soundDir$+"box",volume%,ADDR(soundThread&))
    # 			ENDIF
    # 			BREAK
    # 		ELSEIF o%>=wall% AND this%=bomb%
    # 			sound:(soundDir$+"bomb",volume%,ADDR(soundThread&))
    # 			IF o%>=rock% : q&=q&+cols% : ENDIF
    # 			this%=empty%
    # 			BREAK
    # 		ELSEIF o%=rock%
    # 			IF dir%=0
    # 				IF mx%<x% : dir%=-1 : ELSE : dir%=1 : ENDIF
    # 			ENDIF
    # 			IF x%+dir%<cols% AND x%+dir%>=0 AND PEEKB(q&+dir%)=empty% AND (PEEKB(q&+cols%+dir%)<=empty% OR PEEKB(q&+cols%+dir%)>=box3%)
    # 				x%=x%+dir% : nq&=q&+cols%+dir%
    # 			ELSEIF x%-dir%<cols% AND x%-dir%>=0 AND PEEKB(q&-dir%)=empty% AND (PEEKB(q&+cols%-dir%)<=empty% OR PEEKB(q&+cols%-dir%)>=box3%)
    # 				x%=x%-dir% : nq&=q&+cols%-dir% : dir%=-dir%
    # 			ELSE
    # 				BREAK
    # 			ENDIF
    # 			IF PEEKB(nq&)=matt% : nq&=nq&-cols% : wrong%=KTrue% : ENDIF
    # 			IF PEEKB(nq&)>=box3% : nq&=nq&-cols% : ENDIF
    # 		ELSE
    # 			BREAK
    # 		ENDIF
    # 		q&=nq&
    # 	UNTIL q&>=worldBot&
    # 	sound:(soundDir$+"ston",volume%,ADDR(soundThread&))
    # 	plotBothAddr:(p&,empty%)
    # 	POKEB q&,this% : plotBothAddr:(q&,this%)
    # ENDP
    ...
    def do_move(self) -> None:
        ...
        if block == Tile.FOOD:
            COLLECT_SOUND.play()
            self.food -= 1
        elif block == Tile.STONE:
            new_rockpos = self.hero.position + (self.hero.velocity * 2)
            self.set(new_rockpos, Tile.STONE)
            if self.get(new_rockpos + Vector2(0, 1)) in (
                Tile.EMPTY,
                Tile.BOX1,
                Tile.BOX2,
                Tile.BOX3,
            ):
                self.fall(new_rockpos, newpos, self.hero.velocity.x)
        self.fall(self.hero.position + Vector2(0, -1), newpos, 0)
    ...
    def fall(self, pos: Vector2, hero_pos: Vector2, dx: float) -> None:
        while (
            self.get(pos) in (Tile.STONE, Tile.BOX1, Tile.BOX2, Tile.BOX3)
            and pos.y >= 0
        ):
            self.plummet(pos, hero_pos, dx)
            pos -= Vector2(0, 1)
            dx = 0

    def plummet(self, pos: Vector2, hero_pos: Vector2, push_dx: float) -> None:
        dx = push_dx
        block = self.get(pos)
        print(f"plummet {pos} {push_dx} {block}")
        self.set(pos, Tile.EMPTY)
        while pos.y < level_height:
            block_below = self.get(pos + Vector2(0, 1))
            print(f"{pos=} {pos + Vector2(0, 1)} {block_below=}")
            if block_below == Tile.EMPTY:
                new_pos = pos + Vector2(0, 1)
                if new_pos + Vector2(0, 1) == hero_pos:
                    self.dead = True
            elif block_below in (Tile.BOX1, Tile.BOX2, Tile.BOX3):
                pos += Vector2(0, 1)
                if block == Tile.BOX1:
                    block = Tile.BOX2
                    # FIXME box sound
                elif block == Tile.BOX2:
                    block = Tile.BOX3
                    # FIXME box sound
                elif block == Tile.BOX3:
                    block = Tile.EMPTY
                    # FIXME debox sound
            elif block == Tile.BOMB and block_below in (
                Tile.WALL,
                Tile.STONE,
                Tile.BOMB,
            ):
                # FIXME bomb sound
                if block_below != Tile.WALL:
                    pos += Vector2(0, 1)
                block = Tile.EMPTY
                break
            elif block_below == Tile.STONE:
                if dx == 0:
                    dx = -1 if push_dx > 0 else 1
                if self.get(pos + Vector2(dx, 0)) == Tile.EMPTY and self.get(
                    pos + Vector2(dx, 1)
                ) in (Tile.EMPTY, Tile.BOX1, Tile.BOX2, Tile.BOX3):
                    new_pos = pos + Vector2(dx, 0)
                elif self.get(pos + Vector2(-dx, 0)) == Tile.EMPTY and self.get(
                    pos + Vector2(-dx, 1)
                ) in (Tile.EMPTY, Tile.BOX1, Tile.BOX2, Tile.BOX3):
                    new_pos = pos - Vector2(dx, 0)
                    dx = -dx
                ...
                if new_pos == hero_pos:
                    new_pos -= Vector2(0, 1)
                    self.dead = True
                if self.get(new_pos) in (Tile.BOX1, Tile.BOX2, Tile.BOX3):
                    new_pos -= Vector2(0, 1)
            else:
                break
            pos = new_pos
    ...
    def show_status(self) -> None:
        ...
        screen.blit(FOOD_IMAGE, (1.5 * font_pixels, int(1.5 * font_pixels)))
        print_screen((0, 3), str(self.food), width=100, align="center")
    ...
    def run(self) -> None:
        ...
        while not self.quit and self.level <= levels:
            ...
            self.show_status()
            self.show_screen()
            while not self.quit and not self.dead and self.food > 0:
                self.load_position()
                subframes = 4  # FIXME: global constant
                subframe = 0
                while not self.quit and self.food > 0:
...
class Hero(pygame.sprite.Sprite):  # pylint: disable=too-few-public-methods
    def __init__(self) -> None:
        ...
        self.image = load_image("levels/9-matt.png")
...
def instructions() -> int:
    ...
    instructions = _("""\
Collect all the food on each level.
FIXME: Boxes, bombs.
Avoid falling rocks!
...
def main(argv: List[str] = sys.argv[1:]) -> None:
    ...
    parser = argparse.ArgumentParser(
        description=_(
            "Collect all the food while digging through earth dodging rocks."
        ),
    )
    parser.add_argument(
        "--levels",
    ...
    global COLLECT_SOUND, STONE_SOUND, SPLAT_SOUND
    with importlib_resources.as_file(importlib_resources.files()) as path:
        COLLECT_SOUND = pygame.mixer.Sound(path / "mm_pick.wav")
        COLLECT_SOUND.set_volume(DEFAULT_VOLUME)
        STONE_SOUND = pygame.mixer.Sound(path / "mm_ston.wav")
        STONE_SOUND.set_volume(DEFAULT_VOLUME)
        SPLAT_SOUND = pygame.mixer.Sound(path / "mm_kill.wav")
