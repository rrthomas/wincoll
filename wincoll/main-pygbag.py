...
def print_screen(pos: Tuple[int, int], msg: str, colour: Optional[Union[pygame.Color, str]] = None) -> None:
    font_pixels = 8
    with importlib_resources.as_file(importlib_resources.files()) as path:
...
class WincollGame:
    ...
    def process_move(self) -> None:
        ...
        if block in (self.gids[TilesetGids.GAP], self.gids[TilesetGids.EARTH]):
            ...
            if self.hero.velocity.y == 0 and self.get(new_rockpos) == self.gids[TilesetGids.GAP]:
        ...
        self.set(self.hero.position + self.hero.velocity, self.gids[TilesetGids.WIN_PLACE])

    def can_roll(self, pos: pygame.Vector2) -> bool:
        side_block = self.get(pos)
        side_below_block = self.get(pos + pygame.Vector2(0, 1))
        return side_block == self.gids[TilesetGids.GAP] and side_below_block == self.gids[TilesetGids.GAP]
...
async def main(argv: List[str] = sys.argv[1:]) -> None:
    ...
    with importlib_resources.as_file(importlib_resources.files()) as path:
        COLLECT_SOUND = pygame.mixer.Sound(path / "Collect.ogg")
        SLIDE_SOUND = pygame.mixer.Sound(path / "Slide.ogg")
        UNLOCK_SOUND = pygame.mixer.Sound(path / "Unlock.ogg")
        SPLAT_SOUND = pygame.mixer.Sound(path / "Splat.ogg")
